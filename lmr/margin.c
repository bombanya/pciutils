...
union margin_payload {
  ...
  unsigned int max_timing_steps : 6;
  unsigned int max_voltage_steps : 7;
  unsigned int max_offset : 7;
  unsigned int max_lanes : 5;
  unsigned int sample_rate : 6;

  /* Margin Step Response */
  struct {
    unsigned int err_count : 6;
    unsigned int margin_status : 2;
  };

  /* Timing Step */
  struct {
    unsigned int margin_t_steps : 6;
    bool t_go_left : 1;
  };

  /* Voltage Step */
  struct {
    unsigned int margin_v_steps : 7;
    bool v_go_down : 1;
  };
...
static union margin_cmd
margin_make_cmd(u8 payload, u8 type, u8 recvn)
{
  return (
    union margin_cmd){ .payload.payload = payload, .recvn = recvn, .type = type, .reserved = 0 };
}

static bool
margin_set_cmd(struct margin_dev *dev, u8 lane, union margin_cmd cmd)
{
  pci_write_word(dev->dev, LMR_LANE_CTRL(dev->lmr_cap_addr, lane), cmd.cmd);
  msleep(10);
  return pci_read_word(dev->dev, LMR_LANE_STATUS(dev->lmr_cap_addr, lane)) == cmd.cmd;
}

static bool
margin_report_cmd(struct margin_dev *dev, u8 lane, union margin_cmd cmd,
                  union margin_payload *result)
{
  pci_write_word(dev->dev, LMR_LANE_CTRL(dev->lmr_cap_addr, lane), cmd.cmd);
  msleep(10);
  union margin_cmd resp;
  resp.cmd = pci_read_word(dev->dev, LMR_LANE_STATUS(dev->lmr_cap_addr, lane));
  *result = resp.payload;
  return resp.type == cmd.type && resp.recvn == cmd.recvn && margin_set_cmd(dev, lane, NO_COMMAND);
}
                     ...
                     struct margin_params *params)
{
  ...
  if (status)
    {
      params->volt_support = resp.volt_support;
      params->ind_up_down_volt = resp.ind_up_down_volt;
      params->ind_left_right_tim = resp.ind_left_right_tim;
      params->sample_report_method = resp.sample_report_method;
      params->ind_error_sampler = resp.ind_error_sampler;
      status = margin_report_cmd(dev, lane, REPORT_VOL_STEPS(recvn), &resp);
    }
  if (status)
    {
      params->volt_steps = resp.max_voltage_steps;
      status = margin_report_cmd(dev, lane, REPORT_TIM_STEPS(recvn), &resp);
    }
  if (status)
    {
      params->timing_steps = resp.max_timing_steps;
      status = margin_report_cmd(dev, lane, REPORT_TIM_OFFSET(recvn), &resp);
    }
  if (status)
    {
      params->timing_offset = resp.max_offset;
      status = margin_report_cmd(dev, lane, REPORT_VOL_OFFSET(recvn), &resp);
    }
  if (status)
    {
      params->volt_offset = resp.max_offset;
      status = margin_report_cmd(dev, lane, REPORT_SAMPL_RATE_V(recvn), &resp);
    }
  if (status)
    {
      params->sample_rate_v = resp.sample_rate;
      status = margin_report_cmd(dev, lane, REPORT_SAMPL_RATE_T(recvn), &resp);
    }
  if (status)
    {
      params->sample_rate_t = resp.sample_rate;
      status = margin_report_cmd(dev, lane, REPORT_MAX_LANES(recvn), &resp);
    }
  if (status)
    params->max_lanes = resp.max_lanes;
  return status;
}
  ...
  while (alive_lanes > 0 && steps_done < arg.steps_lane_total)
    {
      ...
      steps_done++;
      if (timing)
        step_cmd.payload.margin_t_steps = steps_done;
      else
        step_cmd.payload.margin_v_steps = steps_done;
          ...
          if (!failed_lanes[i])
            {
              ...
              u8 step_status = lane_status.payload.margin_status;
              if (!(lane_status.type == marg_type && lane_status.recvn == arg.recv->recvn
                    && step_status == 2 && lane_status.payload.err_count <= arg.recv->error_limit
                    && margin_set_cmd(arg.recv->dev, arg.results[i].lane, NO_COMMAND)))
                {
                  alive_lanes--;
                  failed_lanes[i] = true;
